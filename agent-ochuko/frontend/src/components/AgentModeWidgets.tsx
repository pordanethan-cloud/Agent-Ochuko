...
/* ───────────────────────────────────────────────────────────────────────────
   4. INSTANT STATIC SITE DEPLOYMENT PREVIEW CARD (/sites/:slug)
   ─────────────────────────────────────────────────────────────────────────── */
...
export const AgentSiteDeploymentCard: React.FC<AgentSiteDeploymentProps> = ({
  ...
  return (
    <div className="w-full my-3.5 rounded-2xl bg-[#0d0f14] border border-brand-accent/40 shadow-2xl relative overflow-hidden animate-fadeIn select-none">
      {/* Header bar */}
      <div className="p-3.5 sm:p-4 flex items-center justify-between border-b border-white/[0.08] bg-white/[0.02]">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9.5px] font-mono px-1.5 py-0.5 rounded bg-emerald-500/15 text-emerald-400 border border-emerald-500/30">
                LIVE
              </span>
            </div>
            <span className="text-[11px] text-white/50 truncate block font-mono">{previewUrl}</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <button
            ...
            onClick={handleCopy}
            title="Copy URL"
            className="px-2.5 py-1.5 rounded-lg text-[11px] font-medium bg-white/5 hover:bg-white/10 text-white/80 border border-white/10 flex items-center gap-1 transition"
          >
            {copied ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Pencil className="w-3 h-3 text-white/60" />}
          ...
          <a
            ...
            className="px-3 py-1.5 rounded-lg text-[11px] font-bold bg-brand-accent hover:bg-brand-accent/90 text-black flex items-center gap-1 shadow-md shadow-brand-accent/20 transition active:scale-95"
          >
            <span>Open Site</span>
      ...
      {/* Viewport switcher & collapsible toggle */}
      <div className="px-3.5 py-2 bg-black/40 border-b border-white/[0.06] flex items-center justify-between text-[11px] text-white/60">
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={() => setDeviceMode('desktop')}
            className={`px-2 py-0.5 rounded text-[10.5px] transition ${deviceMode === 'desktop' ? 'bg-white/15 text-white font-semibold' : 'text-white/40 hover:text-white/70'}`}
          >
            Desktop View
          </button>
          <button
            ...
            className={`px-2 py-0.5 rounded text-[10.5px] transition ${deviceMode === 'mobile' ? 'bg-white/15 text-white font-semibold' : 'text-white/40 hover:text-white/70'}`}
          >
            Mobile View (375px)
          </button>
        </div>
        <button
          ...
          className="text-[10.5px] text-white/40 hover:text-white/80 transition flex items-center gap-1"
        >
      ...
      {showIframe && (
        <div className="w-full bg-[#050608] flex items-center justify-center p-2 sm:p-4 overflow-x-auto">
          <div
            className={`rounded-xl overflow-hidden border border-white/10 shadow-2xl bg-[#0c0e12] transition-all duration-300 ${
              deviceMode === 'mobile' ? 'w-[375px] h-[550px]' : 'w-full h-[450px]'
            }`}
          >
            <iframe
              src={previewUrl}
